...
// FIXED: entityId was referenced but never destructured from useAuth — now injected by the hook.
// Component owns only form state.
...
export default function ReceiveStock() {
  const {
    ...
    setSubmitError,
    setSubmitSuccess,
  } = useStockEntry({ reasons: ["purchase", "opening_stock"] });
  ...
  const handleSubmit = async () => {
    ...
    if (isNaN(qty) || qty <= 0)  { setSubmitError("Quantity must be a positive number."); return; }

    const ok = await submitMovement({
      ingredient_id:    ingredientId,
      delta:            qty,
      reason,
      unit_cost_at_time: unitCost ? Number(unitCost) : null,
      note:              note.trim() || null,
    });

    if (ok) {
      // Also update ingredient cost_per_unit if a unit cost was provided
      if (unitCost) {
        await supabase
          .from("ingredients")
      ...
      setSubmitSuccess(`✓ Stock recorded — ${qty} ${selectedUnit} added.`);
    ...
    <div style={s.wrap}>
      ...
      <div style={s.formSide}>
        ...
        <button
          ...
          style={{ ...btn.primary, ...btn.sm, opacity: (saving || !ingredientId || !quantity) ? 0.6 : 1 }}
